...
// _Static_assert would compare the condition at compile time
#define xs_tmp(x)                                                   \
    ((void) ((struct {                                              \
...
void xs_copy(xs *dest, xs *src)
{
    if (!src)
        return;
    if (xs_is_large_string(src) && xs_get_refcnt(src) <= MAX_STR_REF) {
        if (xs_is_ptr(dest))
            xs_free(dest);
        *dest = *src;
        xs_inc_refcnt(src);
    } else if (!dest || !xs_is_ptr(dest)) {
        xs_new(dest, xs_data(src));
    } else if (!xs_is_ptr(src)) {
        xs_free(dest);
        dest->is_ptr = false;
        dest->is_large_string = false;
        xs_new(dest, xs_data(src));
    } else {
        if (xs_is_large_string(dest)) {
            xs_free(dest);
            xs_allocate_data(dest, xs_size(src), 0);
        }
        xs_grow(dest, xs_size(src));
        memcpy(xs_data(dest), xs_data(src), xs_size(src));
        xs_data(dest)[xs_size(src)] = 0;
    }
}
